...
function normalizeClusteringSettings(rawSettings = {}, fallback = DEFAULT_CLUSTERING_SETTINGS) {
  ...
  return {
    ...
    hdbscanEffectiveMinSamples: optionalPositiveInteger(
      source.hdbscanEffectiveMinSamples,
      fallback.hdbscanEffectiveMinSamples || "",
    ),
    ...
    hdbscanSampleScope: enumValue(
      source.hdbscanSampleScope,
      ["expanded", "unique"],
      fallback.hdbscanSampleScope || DEFAULT_CLUSTERING_SETTINGS.hdbscanSampleScope,
    ),
    linkage: enumValue(
      source.linkage,
...
export function applyUiPreferences(state) {
  ...
  state.columnsSource = enumValue(
    ...
    ["clusters", "domains", "all"],
    state.columnsSource,
  );
  if (Array.isArray(preferences.columnsCategories)) {
    const allowedCategories = new Set(["contacts", "conservation", "gaps"]);
    state.columnsCategories = new Set(
      preferences.columnsCategories.map(String).filter((value) => allowedCategories.has(value))
    );
  }
  state.columnsRowOrder = enumValue(
    preferences.columnsRowOrder,
    ["group", "category"],
    state.columnsRowOrder,
  );
  state.columnsDisplay = enumValue(
    preferences.columnsDisplay,
    ["lines", "histograms"],
    state.columnsDisplay,
  );
  if (Array.isArray(preferences.columnsPlipTypes)) {
    const allowedPlipTypes = new Set(["1", "2", "4", "8", "16"]);
    state.columnsPlipTypes = new Set(
      preferences.columnsPlipTypes.map(String).filter((value) => allowedPlipTypes.has(value))
    );
  }
  ...
  state.clusterCompareDomainSizeFilter = normalizeDomainSizeRange(
    preferences.clusterCompareDomainSizeFilter,
  );
  state.clusterCompareDomainSizeFilterDraft = normalizeDomainSizeRange(
    preferences.clusterCompareDomainSizeFilterDraft || state.clusterCompareDomainSizeFilter,
  );
