...
export function createClusterCompareController({
  ...
  function entryWithClusterCycleMembers(entry, clusterSummary) {
    ...
    const currentKey = clusterCompareEntryKey(entry);
    const clusterMemberIndex = Math.max(
      0,
      clusterMembers.findIndex((member) => clusterCompareEntryKey(member) === currentKey)
    );
    return {
      ...entry,
      clusterMembers,
  ...
  function clusterCycleControlsHtml(entry, tileIndex, enabled) {
    ...
    const index = Math.max(
      0,
      Math.min(members.length - 1, Number(entry.clusterMemberIndex || 0))
    );
    return `
  ...
  function updateClusterCompareTileMeta(tileIndex) {
    const tile = state.clusterCompareTiles[tileIndex];
    const tileElement = elements.clusterCompareGrid?.querySelector(
      `[data-cluster-compare-index="${tileIndex}"]`
    );
    if (!tile || !tileElement) {
      return;
    }
    ...
    if (count && members.length > 1) {
      const index = Math.max(
        0,
        Math.min(members.length - 1, Number(tile.entry.clusterMemberIndex || 0))
      );
  ...
  async function cycleClusterCompareTileMember(tileIndex, delta) {
    ...
    const currentIndex = Number.isInteger(tile.entry.clusterMemberIndex)
      ...
      : Math.max(
          0,
          members.findIndex((member) => clusterCompareEntryKey(member) === clusterCompareEntryKey(tile.entry))
        );
    ...
    try {
      ...
      const structureResult = await fetchClusterCompareStructure(
        nextEntry,
        nextEntry.rowKey === anchorRowKey ? "" : anchorRowKey
      );
      const stylePayload = representativeClusterCompareTileStyles(
        nextEntry.row,
        nextEntry.clusterSummary,
  ...
  elements.clusterCompareGrid?.addEventListener("click", (event) => {
    ...
    if (cycleButton && elements.clusterCompareGrid.contains(cycleButton)) {
      ...
      void cycleClusterCompareTileMember(
        Number(cycleButton.dataset.clusterCompareCycle),
        Number(cycleButton.dataset.clusterCompareCycleDelta || 0)
      );
      return;
    }
  ...
  async function fetchRepresentativeClusterOverviewSelections(clusters, method) {
    ...
    const selectionsByCluster = new Map(
      (payload.selected_representatives || []).map((selection) => [
  ...
  async function openRepresentativeClusterCompare(options = {}) {
    ...
    try {
      ...
      if (requestId === state.clusterCompareRequestId) {
        setClusterCompareLoading(
          true,
          `Selected overview representatives (${selectedEntries.length}/${clusters.length})...`,
          35
        );
      }
      if (requestId !== state.clusterCompareRequestId) {
