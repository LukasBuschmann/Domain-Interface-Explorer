...
function residuePairLinePdb(modelText, pairs, residueName = "DCL") {
  ...
  for (const [mainResidueId, partnerResidueId, offset = 0] of pairs) {
    ...
    atomLines.push(
      `HETATM${String(firstSerial).padStart(5)} HE   ${residueName} Z${String(residueId).padStart(4)}    ` +
        `${formatPdbNumber(start.x)}${formatPdbNumber(start.y)}${formatPdbNumber(start.z)}  1.00  0.00          He`
    );
    atomLines.push(
      `HETATM${String(secondSerial).padStart(5)} HE   ${residueName} Z${String(residueId).padStart(4)}    ` +
...
function mixedInteractionColor(types) {
  ...
  const channel = (shift) => Math.round(
    colors.reduce((sum, color) => sum + ((color >> shift) & 0xff), 0) / colors.length
  );
...
class DomainMolstarViewer {
  ...
  async loadStructure({
    ...
    try {
      ...
      const trajectory = await this.plugin.builders.structure.parseTrajectory(
        data,
        inferStructureFormat(normalizedModelText, format)
      );
  ...
  async clearRepresentations() {
    ...
    for (const ref of refs) {
      try {
        ...
        // Representation nodes may already be gone after a cancelled or failed update.
      }
    }
  }

  async updateStructureRepresentations({
    payload,
    mode = "structure",
    columnView = false,
    contactsVisible = false,
  ...
  async addRepresentations(options) {
    ...
    const contextResidues = differenceResidues(
  ...
  async addStructureRepresentations(options) {
    ...
    const partnerDomainOnlyResidues = differenceResidues(
      payload?.partner_fragment_residue_ids,
      payload?.partner_surface_residue_ids,
      partnerInterfaceResidues
    );
    if (columnView) {
  ...
  async addPlipInteractionRepresentations(modelText, payload, settings) {
    ...
    for (const group of plipInteractionGroups(payload)) {
      try {
        const renderedPairs = group.pairs.filter(
          ([mainResidueId, partnerResidueId]) =>
            coordinateIds.has(mainResidueId) && coordinateIds.has(partnerResidueId)
        );
        ...
        await this.plugin.builders.structure.representation.addRepresentation(component, {
          ...
          typeParams: typeParamsFor(settings, {
            ...
            extra: {
              visuals: ["intra-bond"],
              sizeFactor: clamp(
                (settings.plipInteractionRadius ?? 0.14) * 36 *
                  (1 + 0.45 * Math.max(0, group.typeCount - 1)),
                1.5,
                12
              ),
              linkScale: 1,
        ...
        // PLIP decoration is optional and should never block structure rendering.
      }
    }
  }
