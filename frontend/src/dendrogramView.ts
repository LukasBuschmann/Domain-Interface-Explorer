...
export function createDendrogramViewController({
  ...
  function renderDendrogram() {
    ...
    setDendrogramInfo(
      `Depth ${state.dendrogram.merge_depth}/${maxDepth} · radius by ${radiusModeLabel} · ${visibleNodeIds.size} visible nodes · ${visibleMemberCount} rows · ${state.dendrogram.cluster_count} clusters · ${state.dendrogram.hierarchy_source}`
    );
  }
