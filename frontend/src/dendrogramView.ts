...
export function createDendrogramViewController({
  state,
  elements,
  interfaceSelect,
  ...
  function pointNearDendrogramCutoff(point, layout) {
    ...
    return (
      Math.abs(distanceFromCenter - screenRadius) <= tolerance &&
  ...
  function sortedClusterLabels(labels) {
    return Array.from(new Set((labels || []).map((label) => String(label)))).sort(
  ...
  function visibleDendrogramClusters() {
    return allDendrogramClusterLabels().filter((clusterKey) =>
      state.dendrogramVisibleClusters.has(clusterKey)
    );
  }

  function visibleDendrogramPartners() {
    return allDendrogramPartnerDomains().filter((partnerDomain) =>
  ...
  function syncDendrogramClusterSelection() {
    ...
    state.dendrogramVisibleClusters = new Set(
      [...state.dendrogramVisibleClusters].filter((clusterKey) => clusterKeys.has(clusterKey))
    );
  }
  ...
  function syncDendrogramControls() {
    ...
    elements.dendrogramSettingsToggle?.setAttribute(
      "aria-expanded",
      String(Boolean(state.dendrogramSettingsOpen))
    );
    ...
    elements.dendrogramTrueDistanceOption?.classList.toggle(
      "disabled",
      controlsDisabled || radiusMode !== "distance"
    );
    if (elements.dendrogramTrueDistanceToggle) {
      elements.dendrogramTrueDistanceToggle.setAttribute(
        "aria-checked",
        String(Boolean(state.dendrogramTrueDistanceEdges))
      );
  ...
  function countVisibleMembers(counts, visibleKeys) {
    ...
    return normalizedCountEntries(counts).reduce(
      (total, [key, count]) => total + (visibleKeys.has(key) ? count : 0),
      0
    );
  }
  ...
  function drawDendrogramTrueDistanceEdge(
    ctx,
    source,
    target,
    sourceNode,
    ...
    const targetScreenRadius = Math.max(
      sourceScreenRadius,
      Math.max(0, target.progress || 0) * layout.radius
    );
  ...
  function drawDendrogramCutoffCircle(ctx, layout) {
    ...
    ctx.fillText(
      `cutoff ${formatDendrogramDistance(cutoffDistance)}`,
      labelX,
      labelY
    );
    ctx.restore();
  ...
  function renderDendrogram() {
    ...
    if (
      elements.dendrogramCanvas.width !== expectedCanvasWidth ||
    ...
    syncDendrogramControls();
    elements.dendrogramLoading?.classList.toggle(
      "hidden",
      !(state.dendrogramLoading && !(state.dendrogram?.nodes || []).length)
    );
    ...
    const visibleKeys = new Set(
      colorMode === "cluster" ? visibleDendrogramClusters() : visibleDendrogramPartners()
    );
    ...
    const trueDistanceEdges =
      dendrogramScaleMode() === "distance" && Boolean(state.dendrogramTrueDistanceEdges);
    for (const edge of state.dendrogram.edges || []) {
      ...
      if (trueDistanceEdges) {
        drawDendrogramTrueDistanceEdge(
          ctx,
          source,
          target,
          sourceNode,
    ...
    const visibleMemberCount = (state.dendrogram.nodes || [])
      ...
      .reduce(
        (total, node) =>
          total +
          countVisibleMembers(
            colorMode === "cluster" ? node.cluster_counts : node.domain_counts,
            visibleKeys
          ),
        0
      );
    setDendrogramInfo(
      `Style ${styleLabel} · depth ${state.dendrogram.merge_depth}/${maxDepth} · scale by ${scaleModeLabel}${distanceLayoutLabel} · ${visibleNodeIds.size} visible nodes · ${visibleMemberCount} rows · ${state.dendrogram.cluster_count} clusters · ${state.dendrogram.hierarchy_source}`
    );
  }
