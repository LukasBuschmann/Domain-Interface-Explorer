...
export function createEmbeddingViewController({ state, elements, interfaceSelect, partnerColor, openColumnsCellStructures = () => { }, renderRepresentativeClusterLegend, renderRepresentativeStructure, syncRepresentativeScopeControls = () => { }, representativeLens, }) {
    ...
    function currentPersistenceScoreMode(settings = state.embeddingClusteringSettingsDraft) {
        ...
        const memoryMode = String(state.embeddingHierarchicalTargetMemory.persistenceScoreMode ??
    ...
    function currentDomainLengthBounds() {
        ...
        const histogram = normalizeDomainLengthHistogram(summary?.domain_length_histogram || optionStats?.domain_length_histogram);
    ...
    function currentColumnsChartQuery() {
        if (columnsSourceMode() === "clusters") {
            const url = new URL(currentEmbeddingClusteringQuery().replace("/api/clustering?", "/api/columns-chart?"), window.location.origin);
    ...
    function syncPersistenceStabilityWeightValueUi() {
        ...
        elements.embeddingClusterStabilityWeightInput.setAttribute("aria-valuetext", `Size ${sizePercent} percent, lifetime ${lifetimePercent} percent`);
    }
    ...
    function readEmbeddingClusteringDraftInputs() {
        ...
        return {
            ...
            hdbscanSampleScope: elements.embeddingClusterHdbscanScopeInput?.value ||
                DEFAULT_CLUSTERING_SETTINGS.hdbscanSampleScope,
    ...
    function syncHierarchicalTargetMemoryFromDraft() {
        ...
        const persistenceLifetimeWeightValue = elements.embeddingClusterStabilityWeightInput.value.trim();
        if (nClustersValue !== "") {
    ...
    function normalizeHierarchicalDraft(settings) {
        ...
        return {
            ...
            persistenceMinLifetime: target === "persistence"
                ...
                : "",
            persistenceLifetimeWeight: target === "persistence"
                ? persistenceLifetimeWeightValue ||
                    state.embeddingHierarchicalTargetMemory.persistenceLifetimeWeight ||
                    String(DEFAULT_CLUSTERING_SETTINGS.persistenceLifetimeWeight)
                : "",
            persistenceScoreMode: target === "persistence"
    ...
    function syncEmbeddingSettingsUi() {
        ...
        elements.clusteringSettingsToggle?.setAttribute("aria-expanded", String(clusteringOpen));
        ...
        if (elements.embeddingClusterHdbscanScopeInput) {
            elements.embeddingClusterHdbscanScopeInput.value = String(state.embeddingClusteringSettingsDraft.hdbscanSampleScope ||
                DEFAULT_CLUSTERING_SETTINGS.hdbscanSampleScope);
        }
        ...
        elements.embeddingClusterStabilityWeightInput.value = String(state.embeddingClusteringSettingsDraft.persistenceLifetimeWeight ||
            state.embeddingHierarchicalTargetMemory.persistenceLifetimeWeight ||
            DEFAULT_CLUSTERING_SETTINGS.persistenceLifetimeWeight);
        syncPersistenceStabilityWeightValueUi();
        const persistenceScoreMode = currentPersistenceScoreMode(state.embeddingClusteringSettingsDraft);
    ...
    function parseEmbeddingClusteringSettingsDraft(options = {}) {
        ...
        if (!allowedClusteringDistances(method).includes(distance)) {
            throw new Error(method === "hdbscan"
        ...
        const hdbscanSampleScope = (elements.embeddingClusterHdbscanScopeInput?.value ||
            DEFAULT_CLUSTERING_SETTINGS.hdbscanSampleScope).trim().toLowerCase();
        ...
        if (method === "hierarchical") {
            if (hierarchicalTarget === "n_clusters") {
                ...
                if (!Number.isFinite(persistenceLifetimeWeight) ||
        ...
        const parsedSettings = {
            ...
            persistenceLifetimeWeight: hierarchicalTarget === "persistence" ? persistenceLifetimeWeight : "",
        ...
        if (options.preserveAppliedHdbscanHierarchy &&
            ...
            parsedSettings.hdbscanEffectiveMinSamples = hdbscanEffectiveMinSamplesForSettings(state.embeddingClusteringSettings);
    ...
    function sizeBracketColor(index, bracketCount) {
        ...
        const paletteIndex = Math.round((Math.max(0, Math.min(bracketCount - 1, index)) * (SIZE_RAINBOW_PALETTE.length - 1)) /
            (bracketCount - 1));
    ...
    function selectedColumnsCategories() {
        ...
        const plipTypes = state.columnsPlipTypes instanceof Set
            ...
            : new Set(["1", "2", "4", "8", "16"]);
        const selectedPlipCategories = COLUMNS_CATEGORIES.filter((category) => (category.key.startsWith("plip:") && plipTypes.has(category.key.slice(5))));
        const selectedPlipMask = selectedPlipCategories.reduce((mask, category) => mask | Number(category.key.slice(5)), 0);
        const categories = [];
    ...
    function columnsInteractionValuesByCluster() {
        ...
        if (columnsInteractionValuesCache.chart === state.columnsChart &&
    ...
    function ensureColumnsChartLoaded() {
        ...
        const promise = (async () => {
            try {
                ...
                if ((clusterSource && requestId !== state.embeddingClusteringRequestId) ||
                    state.columnsChartLoadingKey !== requestKey ||
                    (clusterSource && (!state.embeddingClustering ||
                        state.embeddingClustering.file !== interfaceSelect.value))) {
                    return;
                }
                if (clusterSource) {
    ...
    function columnsVisibleRange(alignmentLength) {
        const total = Math.max(1, Number(alignmentLength) || 1);
        const range = normalizedColumnsRange(state.columnsView?.start ?? 0, state.columnsView?.end ?? total, total);
    ...
    function visibleInterfaceColumns(visibleClusters, range, alignmentLength) {
        ...
        const endColumn = Math.max(startColumn + 1, Math.min(alignmentLength, Math.ceil(range.end)));
        const columns = [];
    ...
    function columnsVirtualWidth(viewportWidth, alignmentLength, span) {
        ...
        return Math.max(viewportWidth, Math.min(2000000, scaledWidth));
    }
    ...
    function payloadHasSelectedInteractionColumn(rowPayload, columnIndex, categoryKey) {
        ...
        return (Array.isArray(rowPayload.plip_interactions) ? rowPayload.plip_interactions : [])
            .some((interaction) => (Array.isArray(interaction) &&
    ...
    function allMembersForColumn(columnIndex, categoryKey) {
        return Object.entries(state.interface?.data || {}).flatMap(([partnerDomain, rowsByKey]) => Object.entries(rowsByKey || {})
            .filter(([_rowKey, rowPayload]) => payloadHasSelectedInteractionColumn(rowPayload, columnIndex, categoryKey))
            .map(([rowKey]) => ({
            row_key: String(rowKey),
            partner_domain: String(partnerDomain),
    ...
    function handleColumnsWheel(event) {
        ...
        event.preventDefault();
        const width = Math.max(1, Math.round((elements.columnsScroll || elements.columnsRoot).clientWidth));
        ...
        if (event.shiftKey || Math.abs(Number(event.deltaX || 0)) > Math.abs(Number(event.deltaY || 0))) {
            if (panColumnsByPixels(Number(event.deltaX || event.deltaY || 0), chartWidth, alignmentLength, visibleClusters)) {
                requestColumnsRenderNextFrame();
            }
            return;
        }
    ...
    function renderColumnsChart() {
        ...
        renderColumnsClusterLegend();
        const showChartLoading = (state.columnsChartLoading || (clusterSource && state.embeddingClusteringLoading)) &&
            !(state.columnsChart?.clusters || []).length;
        elements.columnsLoading.classList.toggle("hidden", !showChartLoading);
            ...
            ctx.fillText(clusterSource && state.embeddingClusteringLoading
                ...
                : clusterSource && state.columnsChartLoading
                    ...
                    : clusterSource
                        ...
                        : state.columnsChartLoading
                            ? "Preparing column histogram..."
                            : `No ${sourcePlural} available.`, centerX, centerY);
        ...
        const rowHeight = Math.max(1, (chartHeight - rowGap * Math.max(0, rowSpecs.length - 1)) / rowSpecs.length);
        ...
        const hiddenColumnCount = Math.max(0, Math.ceil(range.end) - Math.floor(range.start) - interfaceColumns.length);
        ...
        const scaleMeaning = state.columnsIndividualScales
            ...
            : "shared 0-100% scale";
        setColumnsInfo(`${sourceTitle} ${histogramDisplay ? "histograms" : "barcodes"}: ${selectedCategories.length} values, ${groupFirst ? "group-first" : "category-first"}; ${modeLabel}; ${colorMeaning}, ${scaleMeaning} (${visibleClusters.length}/${state.columnsChart.clusters.length} ${sourcePlural} visible, columns ${startLabel}-${endLabel}).`);
    }
