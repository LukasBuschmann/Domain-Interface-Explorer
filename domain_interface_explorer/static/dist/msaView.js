...
export function createMsaViewController({ state, elements, buildPairs, activeConservationVector, conservationColor, overlayStateForRow, representativeLens, embeddingDistanceLabel, syncColumnLegends, syncRepresentativeLensControls, syncRepresentativeScopeControls, syncEmbeddingLoadingUi, syncEmbeddingMemberControls, syncEmbeddingSettingsUi, resizeEmbeddingCanvas, resizeColumnsCanvas, resizeDendrogramCanvas, renderEmbeddingPlot, renderColumnsChart, renderDendrogram, renderDendrogramLegend, renderColumnsClusterLegend, setEmbeddingInfo, setColumnsInfo, ensureEmbeddingDataLoaded, ensureEmbeddingClusteringLoaded, ensureDendrogramLoaded, resetColumnsClusterSelection, resetDendrogramPartnerSelection, resetDendrogramClusterSelection, resetEmbeddingPartnerSelection, resetEmbeddingClusterSelection, resetEmbeddingMetricSelections, resetRepresentativePartnerSelection, resetRepresentativeClusterSelection, renderRepresentativePartnerFilter, renderEmbeddingLegend, refreshRepresentativeSelection, loadInteractiveStructure, handleStructureLoadFailure, resetRepresentativePanel, resetStructurePanel, closeClusterCompareModal, closeStructureModal, resizeClusterCompareViewers, buildOverlayMaps, buildPartnerColorMap, embeddingClusterColor, embeddingClusterLabel, allColumnsClusterLabels, visibleColumnsClusters, updatePartnerOptions, syncDendrogramControls, syncDomainSizeRangeForSelectedFamily, }) {
    ...
    function normalizeInterfaceSummary(summary) {
        ...
        const rawDomainLengthHistogram = summary.domain_length_histogram || summary.domainLengthHistogramEntries;
        const rawPfamRowCoverageHistogram = summary.pfam_row_coverage_histogram || summary.pfamRowCoverageHistogramEntries;
        const rawPlipInteractionCountHistogram = summary.plip_interaction_count_histogram || summary.plipInteractionCountHistogramEntries;
        ...
        const plipInteractionCountHistogramEntries = normalizeHistogramEntries(rawPlipInteractionCountHistogram, true);
    ...
    function localInterfaceSummary(interfacePayload, selectedPartner = state.selectedPartner) {
        ...
        for (const entry of entries) {
            ...
            if (pfamRowCoverage > 0) {
                countsByPfamRowCoverage.set(pfamRowCoverage, (countsByPfamRowCoverage.get(pfamRowCoverage) || 0) + 1);
            }
        ...
        return {
            ...
            domainLengthHistogramEntries: normalizeHistogramEntries([...countsByDomainLength.entries()].map(([size, count]) => ({ size, count }))),
            pfamRowCoverageHistogramEntries: normalizeHistogramEntries([...countsByPfamRowCoverage.entries()].map(([size, count]) => ({ size, count }))),
        };
    }
    ...
    function histogramTargetsForBin(bin, valueFromEntry = (_partnerDomain, _rowKey, rowPayload) => interfaceSizeFromPayload(rowPayload)) {
        const targets = [];
    ...
    async function showRandomInterfaceForHistogramBin(bin, targetType, emptyMessage = "No filtered interfaces are available in that histogram range.") {
        try {
    ...
    async function showInterfacesForHistogramBin(bin, targetType, emptyMessage = "No filtered interfaces are available in that histogram range.") {
        try {
            const candidates = (await loadHistogramTargetsForBin(bin, targetType)).sort((left, right) => String(left.rowKey).localeCompare(String(right.rowKey)));
            if (candidates.length === 0) {
                appStatus.textContent = emptyMessage;
                hideLoading();
                return;
    ...
    function renderPfamInfoHistogram({ titleText, histogramAvailable, histogramEntries, unavailableText, emptyText, countSingular, countPlural, targetType, noTargetsText, valueFormatter = (value) => String(value), valueUnitLabel = "residues", }) {
        ...
        for (const bin of chartBins) {
            ...
            column.title = `${binLabel}${valueUnitLabel ? ` ${valueUnitLabel}` : ""}: ${histogramCountLabel(bin.count, countSingular, countPlural)}`;
    ...
    function updatePfamOptionDisplayNames(metadataByPfam) {
        ...
        const normalizedMetadata = new Map(Object.entries(metadataByPfam)
    ...
    async function pollPfamMetadataIfNeeded() {
        ...
        pfamMetadataPollHandle = window.setTimeout(async () => {
            ...
            try {
                ...
                const statsRefreshFinished = wasStatsRefreshing &&
                    nextStatsStatus &&
