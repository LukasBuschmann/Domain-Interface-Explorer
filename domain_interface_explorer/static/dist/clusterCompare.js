...
export function createClusterCompareController({ state, elements, interfaceSelect, currentClusterCompareQuery, getRowByKey, embeddingClusterLabel, embeddingDistanceLabel, nextBrowserPaint, openStructureForEntry, }) {
    ...
    async function openClusterCompareForLabel(clusterLabel, options = {}) {
        ...
        const keepExistingTilesDuringLoad = Boolean(state.clusterCompareTiles.length > 0 &&
        ...
        try {
            ...
            const results = await Promise.all(selectedEntries.map(async (entry) => {
                try {
                    return await fetchClusterCompareStructure(entry, entry.rowKey === anchorRowKey ? "" : anchorRowKey);
                }
                catch (error) {
                    return {
                        entry,
