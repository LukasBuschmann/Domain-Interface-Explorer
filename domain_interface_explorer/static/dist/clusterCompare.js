...
export function createClusterCompareController({ state, elements, interfaceSelect, currentClusterCompareQuery, getRowByKey, embeddingClusterLabel, embeddingDistanceLabel, nextBrowserPaint, openStructureForEntry, openClusterResidueMembers, representativeClusterCompareSummaries = () => [], representativeClusterCompareUrl = () => "", representativeClusterOverviewUrl = () => "", normalizeRepresentativeRow = (row) => row, representativeClusterSummaryFromPayload = (_payload, _clusterLabel, fallbackSummary) => fallbackSummary, structureDisplaySettingsForView = () => state.structureDisplaySettings, representativeClusterCompareTileStyles = () => ({
    ...
    function setClusterCompareDomainSizeMenuOpen(open) {
        ...
        if (open) {
            const range = normalizedClusterCompareDomainSizeFilter(state.clusterCompareDomainSizeFilterDraft || state.clusterCompareDomainSizeFilter || {});
        ...
        elements.clusterCompareDomainSizeButton.setAttribute("aria-expanded", open ? "true" : "false");
    }
    ...
    function setClusterCompareRepresentativeMethodMenuOpen(open) {
        ...
        elements.clusterCompareRepresentativeMethodButton.setAttribute("aria-expanded", open ? "true" : "false");
    }
    function syncClusterCompareHeaderControls() {
        ...
        elements.clusterCompareRepresentativeMethodControl?.classList.toggle("hidden", !isRepresentativeClusterMode);
        elements.clusterCompareRerollButton?.classList.toggle("hidden", isRepresentativeClusterMode);
        ...
        if (elements.clusterCompareRepresentativeMethodMenu) {
            [
                ...elements.clusterCompareRepresentativeMethodMenu.querySelectorAll("[data-cluster-compare-representative-method]"),
            ].forEach((button) => {
    ...
    function clusterCompareRecordHasModelData(record) {
        return (record?.results || []).every((result) => result?.error ||
            (result?.payload &&
    ...
    function setClusterCompareGridLoading(visible) {
        ...
        elements.clusterCompareGrid.classList.toggle("is-loading", Boolean(visible && elements.clusterCompareGrid.children.length > 0));
    }
    ...
    function setClusterCompareLoading(visible, label = "Loading aligned structures...", progress = null) {
        ...
        if (elements.clusterCompareLoadingBar) {
            const clampedProgress = progress === null || progress === undefined
                ? 1
    ...
    function applyClusterCompareGridLayout(tileCount) {
        ...
        const columns = tileCount >= 5 ? 3 : tileCount === 4 ? 2 : tileCount >= 3 ? 3 : Math.max(1, tileCount);
    ...
    function currentClusterCompareView() {
        ...
        const tileWithView = state.clusterCompareTiles.find((tile) => tile?.viewer && typeof tile.viewer.getView === "function");
    ...
    async function openClusterCompareTileStructure(tileIndex) {
        ...
        const initialView = tile.viewer && typeof tile.viewer.getView === "function"
    ...
    function clusterCompareDomainSelection(payload) {
        return {
            resi: (Array.isArray(payload?.fragment_residue_ids) && payload.fragment_residue_ids.length > 0)
                ? payload.fragment_residue_ids
                : Array.from({ length: payload.fragment_end - payload.fragment_start + 1 }, (_value, index) => payload.fragment_start + index),
        };
    }
    function centerClusterCompareDomains(options = {}) {
        const anchorTile = state.clusterCompareTiles.find((tile) => tile?.viewer && tile?.payload && !tile.error);
    ...
    function shouldRetryClusterCompareModelLoad(error, tile) {
        ...
        return Boolean(tile?.payload?.model_url &&
    ...
    function initializeClusterCompareTileViewers() {
        return Promise.allSettled(state.clusterCompareTiles.map(async (_tile, tileIndex) => {
    ...
    function refreshRepresentativeClusterTileStyles(tile) {
        ...
        const stylePayload = representativeClusterCompareTileStyles(tile.row, tile.entry.clusterSummary, tile.payload);
    ...
    function clusterCompareSubtitle(record) {
        ...
        return (`Showing ${record.selectedEntries.length} of ${record.clusterEntryCount} cluster entries, ` +
    ...
    function renderClusterCompareGridShell(record) {
        ...
        elements.clusterCompareGrid.innerHTML = selectedEntries
            ...
            const swatch = entry.color
                ...
                : "";
            const coverageTitle = escapeHtml(entry.coverageTitle ||
            ...
            const coverageLabel = escapeHtml(entry.coverageLabel || `${roundedPercent(entry.coveragePercent)}%`);
            return `
    ...
    function setClusterCompareTilesFromResults(results) {
        state.clusterCompareTiles = results.map((result, index) => ({
            ...
            viewer: null,
            cleanupSync: null,
            viewerRoot: elements.clusterCompareGrid.querySelector(`[data-cluster-compare-viewer="${index}"]`),
        }));
    }
    function prepareClusterCompareTiles(record) {
        renderClusterCompareGridShell(record);
        setClusterCompareTilesFromResults((record.selectedEntries || []).map((entry) => ({ entry })));
    ...
    async function openClusterCompareForLabel(clusterLabel, options = {}) {
        ...
        const keepExistingTilesDuringLoad = Boolean(previousMode === "cluster" &&
            state.clusterCompareTiles.length > 0 &&
            elements.clusterCompareGrid?.children.length);
        const previousAlignmentAnchorRowKey = keepExistingTilesDuringLoad && previousClusterLabel === targetClusterLabel
        ...
        setClusterCompareLoading(true, cachedRecord ? "Restoring cached comparison..." : "Selecting diverse interfaces...", cachedRecord ? 80 : 12);
        try {
            ...
            const clusterCompareData = await fetchClusterCompareData(clusterLabel, clusterCompareFetchUrl(cacheKey, reroll));
            if (requestId !== state.clusterCompareRequestId) {
                return;
            }
            ...
            const selectedEntries = [...(clusterCompareData.selectedEntries || [])].sort((left, right) => right.coveragePercent - left.coveragePercent ||
                left.selectionRank - right.selectionRank ||
                left.rowKey.localeCompare(right.rowKey) ||
                left.partnerDomain.localeCompare(right.partnerDomain));
            const distanceMetric = clusterCompareData.distance || state.embeddingClusteringSettings.distance || "overlap";
            ...
            setClusterCompareLoading(true, `Loading aligned structures (0/${selectedEntries.length})...`, 1);
            if (!keepExistingTilesDuringLoad) {
                prepareClusterCompareTiles(record);
            }
            let completedStructures = 0;
            const results = await Promise.all(selectedEntries.map(async (entry, index) => {
                let result;
                try {
                    result = await fetchClusterCompareStructure(entry, entry.rowKey === anchorRowKey ? "" : anchorRowKey);
                }
                catch (error) {
                    result = {
                        entry,
                    ...
                    if (requestId === state.clusterCompareRequestId) {
                        setClusterCompareLoading(true, `Loading aligned structures (${completedStructures}/${selectedEntries.length})...`, Math.round((completedStructures / selectedEntries.length) * 100));
                    }
                }
    ...
    async function openRepresentativeClusterCompare(options = {}) {
        ...
        state.clusterCompareRequestId = requestId;
        const preservedView = currentClusterCompareView();
        const keepExistingTilesDuringLoad = Boolean(previousMode === "representative-clusters" &&
            state.clusterCompareTiles.length > 0 &&
            elements.clusterCompareGrid?.children.length &&
            state.clusterCompareMode === "representative-clusters");
        if (preservedView) {
        ...
        setClusterCompareLoading(true, cachedRecord ? "Restoring cached Cluster Overview..." : "Selecting cluster overview representatives...", cachedRecord ? 80 : 10);
        try {
            await nextBrowserPaint();
            if (cachedRecord) {
                if (requestId !== state.clusterCompareRequestId) {
            ...
            const record = {
                ...
                title,
                subtitle: `Showing the ${selectedEntries.length} largest clusters. Representatives selected by ${representativeMethodLabel(method)}.`,
                representativeMethod: method,
                selectedEntries,
                clusterEntryCount: clusters.reduce((sum, cluster) => sum + Number(cluster.compareMemberCount ?? cluster.memberCount ?? 0), 0),
                remainingEntryCount: clusters.reduce((sum, cluster) => sum + Number(cluster.compareMemberCount ?? cluster.memberCount ?? 0), 0),
            ...
            const results = await Promise.all(selectedEntries.map(async (entry, index) => {
                ...
                try {
                    ...
                    completedStructures += 1;
                    if (requestId === state.clusterCompareRequestId) {
                        setClusterCompareLoading(true, `Loading aligned structures (${completedStructures}/${selectedEntries.length})...`, 35 + Math.round((completedStructures / selectedEntries.length) * 65));
                    }
                }
                if (!keepExistingTilesDuringLoad && requestId === state.clusterCompareRequestId) {
                    await applyClusterCompareTileResult(index, result);
    ...
    elements.clusterCompareDomainSizeButton?.addEventListener("click", () => {
        setClusterCompareRepresentativeMethodMenuOpen(false);
        setClusterCompareDomainSizeMenuOpen(elements.clusterCompareDomainSizeMenu?.classList.contains("hidden"));
    });
    ...
    elements.clusterCompareRepresentativeMethodButton?.addEventListener("click", () => {
        setClusterCompareDomainSizeMenuOpen(false);
        setClusterCompareRepresentativeMethodMenuOpen(elements.clusterCompareRepresentativeMethodMenu?.classList.contains("hidden"));
    });
