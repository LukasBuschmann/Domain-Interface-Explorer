...
export function createStructureViewController({ state, elements, THREE_TO_ONE, interfaceSelect, setLoading, hideLoading, buildStructureResidueLookup, columnResidueStyles, structureMarkerResidueStyles = () => [], msaColumnMaxIndex, topResiduesForColumn, columnStateDistribution, syncColumnLegends, getSelectedRow, getStructurePreloadRows, clearEmbeddingMemberSelection, partnerColor = () => "#817a71", onResidueClick = null, structureDisplaySettingsForView = () => state.structureDisplaySettings, }) {
    ...
    function overlapPercentForItem(item) {
        return percentFromItem(item, ["overlap_hmm_coverage_percent"], ["overlap_hmm_coverage"]);
    }
    function unionPercentForItem(item) {
        return percentFromItem(item, ["union_hmm_coverage_percent"], ["union_hmm_coverage"]);
    }
    function mainUnionGainPercentForItem(item) {
        return percentFromItem(item, ["main_union_hmm_coverage_gain_percent", "main_union_coverage_gain_percent"], ["main_union_hmm_coverage_gain", "main_union_coverage_gain"], { clamp: false });
    }
    function partnerUnionGainPercentForItem(item) {
        return percentFromItem(item, ["partner_union_hmm_coverage_gain_percent", "partner_union_coverage_gain_percent"], ["partner_union_hmm_coverage_gain", "partner_union_coverage_gain"], { clamp: false });
    }
    ...
    function renderStructureHmmUnionGainBars(payload) {
        ...
        for (const entry of gainItems) {
            ...
            const detail = document.createElement("span");
            detail.textContent = hmmUnionGainDetailLabel(entry.item, entry.domainLabel, entry.domainCovered);
            label.append(labelTitle, detail);
            const track = document.createElement("div");
            track.className = "structure-hmm-union-gain-track";
            track.title = hmmUnionGainTrackTitle(entry.item, entry.domainLabel, entry.domainCovered, entry.unionGain, entry.unionPercent);
            if (Number.isFinite(entry.unionGain) && entry.unionGain > 0) {
                const fill = document.createElement("span");
                fill.className = `structure-hmm-union-gain-fill ${entry.index === 0
    ...
    async function updateStructureIdleStats(row, payload) {
        ...
        try {
            ...
            if (state.structureData?.row !== row)
                return;
    ...
    async function updatePlipHoverInformation(residueId, columnIndex, payload, selectedResidue = "") {
        ...
        try {
            ...
            setStructureHoverHistogram(topResiduesForColumn(numericColumnIndex, selectedResidue, distribution));
            setStructureHoverDistribution(columnStateDistribution(numericColumnIndex, undefined, distribution));
    ...
    function handleStructureHover(hoverPayload) {
        if (hoverPayload?.kind === "plip") {
            ...
            void updatePlipHoverInformation(mainResidueId, mapped?.columnIndex ?? null, state.structureData?.payload, mapped?.aminoAcid || "");
            return;
        }
        elements.structurePlipHoverCard.classList.add("hidden");
        ...
        setStructureHoverHistogram(null);
        setStructureHoverDistribution(null);
        void updatePlipHoverInformation(residueId, hover.columnIndex, state.structureData?.payload, hover.residueLetter);
    }
