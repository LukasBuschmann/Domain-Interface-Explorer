...
export function createRepresentativeViewController({ state, elements, THREE_TO_ONE, interfaceSelect, syncColumnLegends, msaColumnMaxIndex, ensureEmbeddingClusteringLoaded, representativeClusterLensData, representativeResidueStyles, renderRepresentativeClusterLegend, partnerInteractionDistribution, buildStructureResidueLookup, representativeLens, getRepresentativeRow, clusteringMethodLabel, allRepresentativeClusterLabels, visibleRepresentativeClusters, partnerColor, }) {
    ...
    function positionRepresentativeHoverCard() {
        ...
        if (representativeLens() === "cluster") {
            ...
            const top = Math.max(12, viewerTopInStage + viewerRect.height - elements.representativeHoverCard.offsetHeight - 12);
        ...
        const maxLeft = Math.max(12, viewerLeftInStage + viewerRect.width - elements.representativeHoverCard.offsetWidth - 12);
        const maxTop = Math.max(12, viewerTopInStage + viewerRect.height - elements.representativeHoverCard.offsetHeight - 12);
        const left = Math.min(maxLeft, Math.max(12, viewerLeftInStage + state.representativePointer.x - viewerRect.left + offset));
        const top = Math.min(maxTop, Math.max(12, viewerTopInStage + state.representativePointer.y - viewerRect.top + offset));
        elements.representativeHoverCard.style.left = `${left}px`;
        elements.representativeHoverCard.style.top = `${top}px`;
    }
    ...
    function clearRepresentativeClusterHover(rerender = true) {
        ...
        if (hadHoveredCluster && rerender && state.representativeStructure && representativeLens() === "cluster") {
            renderRepresentativeClusterLegend(representativeClusterLensData(state.representativeStructure.row));
        }
    }
    function handleRepresentativeHover(hoverPayload, row, clusterLensData = null) {
        ...
        if (representativeLens() === "cluster") {
            ...
            const cluster = clusterLensData?.clusters?.find((entry) => entry.clusterLabel === clusterResidue.clusterLabel);
            ...
            setRepresentativeHoverDistribution(clusterResidue.distribution, "Percent Of Domain Rows In Cluster", "Hover a representative cluster region.");
    ...
    async function renderRepresentativeStructure() {
        ...
        const clusterLensData = representativeLens() === "cluster" ? representativeClusterLensData(representative.row) : null;
        const residueStyles = representativeResidueStyles(representative.row, clusterLensData);
        const shouldPreserveView = state.representativeRenderedRowKey === representative.row.row_key &&
    ...
    async function loadRepresentativeStructure() {
        ...
        if (state.representativeStructure?.row?.row_key === row.row_key &&
        ...
        const alignmentReferenceRowKey = state.representativeAnchorRowKey && state.representativeAnchorRowKey !== requestRowKey
