import { CELL_WIDTH, CLUSTER_COLOR_PALETTE, DEFAULT_CLUSTERING_SETTINGS, DEFAULT_EMBEDDING_SETTINGS, DEFAULT_STRUCTURE_DISPLAY_SETTINGS, DEFAULT_STRUCTURE_DISPLAY_VIEW_PRESETS, HEADER_HEIGHT, LABEL_WIDTH, PARTNER_COLOR_PALETTE, ROW_HEIGHT, TEXT_FONT, THREE_TO_ONE, } from "./constants.js";
...
async function activateEmbeddingPoint(event, source = "unknown") {
    ...
    const point = embeddingPointAt(event.clientX, event.clientY) ||
    ...
    if (!row) {
        ...
        const interfaceRowExists = allRows.some((entry) => String(entry.interface_row_key || "") === parsedTarget.interfaceRowKey);
...
function hasCurrentEmbeddingClustering() {
    return Boolean(interfaceSelect.value &&
...
function syncColumnLegends() {
    syncColumnLegend(structureColumnLegend, structureColumnLegendStart, structureColumnLegendMid, structureColumnLegendEnd, state.structureColumnView);
    syncColumnLegend(representativeColumnLegend, representativeColumnLegendStart, representativeColumnLegendMid, representativeColumnLegendEnd, representativeLens() === "column");
...
function shouldShowRepresentativePartnerFilter() {
    return (representativeLens() === "partners" &&
...
function syncRepresentativeScopeControls() {
    ...
    if (representativeScopeMenu.dataset.optionsKey !== optionsKey) {
      ...
      >
        ...
        <span>Overall</span>
            ...
            ...clusterLabels.map((clusterLabel) => `<button
            type="button"
            class="representative-scope-option"
    ...
    const swatchColor = selectedClusterLabel === null ? "#817a71" : embeddingClusterColor(selectedClusterLabel);
    if (representativeScopeSwatch) {
    ...
    [...representativeScopeMenu.querySelectorAll("[data-representative-scope-value]")].forEach((button) => {
...
function syncRepresentativeMethodControls() {
    ...
    if (representativeMethodButton) {
        const menuOpen = representativeMethodMenu
            ...
            : false;
        representativeMethodButton.setAttribute("aria-expanded", menuOpen ? "true" : "false");
    }
    if (!representativeMethodMenu) {
        return;
    }
    [...representativeMethodMenu.querySelectorAll("[data-representative-method]")].forEach((button) => {
...
function appendClusteringSettingsToParams(params) {
    ...
    if (state.embeddingClusteringSettings.method === "hierarchical") {
        ...
        const minClusterSize = String(state.embeddingClusteringSettings.hierarchicalMinClusterSize ??
            DEFAULT_CLUSTERING_SETTINGS.hierarchicalMinClusterSize).trim();
        if (minClusterSize !== "") {
...
function representativeResidueStyles(row, clusterLensData = null) {
    ...
    for (const entry of residueLookup.values()) {
        ...
        if (mode === "interface") {
            ...
            intensity = Math.max(0, Math.min(1, Number(activeConservationVector()[entry.columnIndex] || 0) / 100));
...
function representativeClusterCompareSummaries() {
    ...
    const summaries = representativeClusterSummaries()
        ...
        .map((summary) => {
        const compareMemberCount = state.selectedPartner === "__all__"
    ...
    const totalMemberCount = summaries.reduce((sum, summary) => sum + Number(summary.compareMemberCount || 0), 0);
    return summaries
        .map((summary) => {
        const coverageFraction = totalMemberCount > 0 ? Number(summary.compareMemberCount || 0) / totalMemberCount : 0;
        return {
            ...summary,
            totalMemberCount,
            coverageFraction,
        ...
        .sort((left, right) => right.compareMemberCount - left.compareMemberCount ||
...
function representativeClusterCompareTileStyles(row, clusterSummary, structurePayload = null) {
    ...
    const interfaceResidueIds = new Set((Array.isArray(structurePayload?.interface_residue_ids)
...
function representativeClusterSummaryFromPayload(payload, clusterLabel, fallbackSummary = null) {
    const summaries = Array.isArray(payload?.cluster_summaries)
        ? payload.cluster_summaries
            ...
            .filter(Boolean)
        : [];
    return (summaries.find((summary) => String(summary.clusterLabel) === String(clusterLabel)) ||
        fallbackSummary);
}
...
function representativeClusterLensData(row) {
    ...
    const clusters = new Map(clusterSummaries.map((summary) => [
...
function syncRepresentativeClusterCompareButton(clusters = null) {
    ...
    const visible = representativeLens() === "cluster" &&
        clusterCount > 0 &&
        !state.embeddingClustering?.error;
...
function updatePartnerOptions() {
    ...
    const options = [{ value: "__all__", label: "All partners" }].concat(partners.map((partner) => ({ value: partner, label: partner })));
...
function clusterOverviewResidueMembers(clusterLabel, columnIndex) {
    ...
    for (const point of state.embeddingClustering?.points || []) {
        ...
        for (const member of normalizedClusterPointMembers(point)) {
            ...
            const hasColumn = interfaceColumns instanceof Set &&
    ...
    return members.sort((left, right) => left.partner_domain.localeCompare(right.partner_domain) ||
        left.row_key.localeCompare(right.row_key));
}
async function openClusterOverviewResidueMembers({ clusterLabel, columnIndex, residueId, residueName = "", entry = null, } = {}) {
    ...
    if (members.length === 0) {
        ...
        throw new Error(`No ${embeddingClusterLabel(clusterLabel)} interfaces interact at ${residueLabel} / MSA column ${columnIndex}.`);
    }
...
async function openStructureForInteractionEntry(entry, loadedStructure = {}) {
    const rowKey = interactionRowKey(entry?.rowKey || entry?.row_key || "", entry?.partnerDomain || entry?.partner_domain || "");
    ...
    if (clusterMembers.length > 1) {
        const members = clusterMembers
            ...
            .filter((member) => member.row_key && member.partner_domain);
        const selectedIndex = Math.max(0, members.findIndex((member) => embeddingMemberKey(member) === rowKey));
        state.embeddingMemberSelection = {
...
const { clearRepresentativeClusterHover, getRepresentativeViewer, loadRepresentativeStructure, positionRepresentativeHoverCard, renderRepresentativePartnerFilter, resetRepresentativePanel, syncRepresentativeLensControls, syncRepresentativePartnerFilterVisibility, } = representativeViewController;
...
function uniqueStructureDisplayPresetId(name, existingId = "") {
    const usedIds = new Set((state.structureDisplayCustomPresets || [])
...
const STRUCTURE_REGION_SETTING_KEYS = new Set(STRUCTURE_REGION_CONTROLS.flatMap((region) => [region.styleKey, region.colorKey, region.alphaKey]));
...
function renderStructureRegionControls() {
    ...
    const styleOptions = STRUCTURE_REGION_STYLE_OPTIONS.map(([value, label]) => `<option value="${value}">${label}</option>`).join("");
...
function setStructureDisplaySettingsOpen(open) {
    ...
    structureDisplaySettingsPanel?.setAttribute("aria-hidden", state.structureDisplaySettingsOpen ? "false" : "true");
    if (nextOpen && !wasOpen) {
...
function syncStructureDisplaySettingsUi() {
    ...
    for (const control of structureDisplayControls()) {
        ...
        if (control.type === "checkbox") {
            ...
            control.value = colorInputValue(settings[key], DEFAULT_STRUCTURE_DISPLAY_SETTINGS[key] || "#ffffff");
        }
...
function currentStructureDisplayPresetViewKeys() {
    return STRUCTURE_VIEW_PRESET_KEYS.filter((viewKey) => structureViewPresetValue(viewKey) === STRUCTURE_VIEW_PRESET_CURRENT);
}
...
function scheduleStructureDisplayRepresentationRefresh(targets) {
    ...
    structureDisplayRefreshFrame = window.setTimeout(() => {
        const refreshTargets = new Set(structureDisplayRefreshTargets.size > 0
        ...
        if (refreshTargets.has("structure") &&
            state.structureData &&
            structureModal &&
...
function onGridHover(event) {
    ...
    setDetails({
        ...
        column,
        conservedness: activeConservationVector()[column] === undefined
            ? "-"
...
selectionSettingsApply?.addEventListener("click", async () => {
    try {
        ...
        const settingsChanged = nextSettings.minInterfaceSize !== state.selectionSettings.minInterfaceSize;
...
function openEmbeddingSettingsSection(section) {
    ...
    if (String(state.embeddingClusteringSettings.distanceThreshold).trim() !== "") {
        state.embeddingHierarchicalTargetMemory.distanceThreshold = String(state.embeddingClusteringSettings.distanceThreshold).trim();
    }
...
embeddingPartnerLegend.addEventListener("click", (event) => {
    ...
    if (clusterButton) {
        ...
        if (event.ctrlKey || event.metaKey) {
            const isIsolated = state.embeddingVisibleClusters.size === 1 &&
    ...
    if (event.ctrlKey || event.metaKey) {
        const isIsolated = state.embeddingVisiblePartners.size === 1 &&
...
representativeClusterLegend?.addEventListener("click", (event) => {
    ...
    if (event.ctrlKey || event.metaKey) {
        const isIsolated = state.representativeVisibleClusters.size === 1 &&
...
representativePartnerFilterList.addEventListener("click", (event) => {
    ...
    const allPartners = state.interface?.partnerDomains || [];
    if (event.ctrlKey || event.metaKey) {
        const isIsolated = state.representativeVisiblePartners.size === 1 &&
...
window.addEventListener("mousemove", (event) => {
    if (state.embeddingDrag) {
        ...
        state.embeddingView.pitch = Math.max(-1.35, Math.min(1.35, state.embeddingDrag.pitch + deltaY * 0.01));
        requestEmbeddingRender();
        return;
    }
    if (activeMsaPanelView() !== "embeddings") {
